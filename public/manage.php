...
if (isset($_GET['delete'])) {
    ...
    header('Location: manage.php?success=Assignment+deleted');
    exit;
}
                ...
                <form method="POST" action="manage.php" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <?php if ($editAssignment && !empty($editAssignment['attachment'])): ?>
                            <span class="form-hint">Current: <a href="download.php?file=<?= urlencode($editAssignment['attachment']) ?>&type=assignment" target="_blank">Download</a>. Uploading a new file will overwrite it.</span>
                        <?php else: ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php if ($editAssignment && !empty($editAssignment['master_correction'])): ?>
                            <span class="form-hint">Current: <a href="download.php?file=<?= urlencode($editAssignment['master_correction']) ?>&type=master_correction" target="_blank">Download Master Correction</a>. Uploading a new file will overwrite it.</span>
                        <?php else: ?>
                            ...
                            <div style="flex:1">
                                ...
                                <?php if (!empty($a['attachment'])): ?>
                                    <div style="margin-top: 0.5rem; font-size: 0.8rem;">
                                        📄 <a href="download.php?file=<?= urlencode($a['attachment']) ?>&type=assignment" target="_blank" style="color: var(--primary); font-weight: 500;">View Attachment</a>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div style="display:flex;gap:.5rem;flex-shrink:0">
                                <a href="manage.php?edit=<?= (int)$a['id'] ?>" class="btn btn-secondary btn-sm">Edit</a>
                                <a href="manage.php?delete=<?= (int)$a['id'] ?>" class="btn btn-danger btn-sm"
